...
n_projection = 1- a_forward_trace["fraction"];
...
level.m_cough_rig.angles = level.m_carry_rig.angles;
level.m_cough_rig.angles = level.m_player_spot.angles;
...
level.m_player_spot RotateTo( VectorToAngles( v_player_to_spot ), n_rotate_speed, n_rotate_speed *.25 );
}
soap_cough_blood()
{
level.soap dialogue_queue( "presc_mct_fallpains" );
level.player PlayRumbleOnEntity( "damage_heavy" );
wait ( 2 );
level.player PlayRumbleOnEntity( "damage_light" );
PlayFXOnTag( level._effect[ "blood_cough_heavy" ], level.soap, "tag_eye" );
level.price delaythread( 2, ::dialogue_queue, "presc_pri_getoffroad" );
}
...
m_smoke_left Unlink();
wait( 14 );
trigger_on( "soap_carry_after_smoke_trig", "targetname" );
}
price_grenade_wall()
{
...
{
level.price dialogue_queue( "presc_pri_dontyoudare" );
}
