...
registerActionBinding( "sprint_pc", "+sprint_or_jump", &"INTRO_HINT_SPRINT_WII" );
registerActionBinding( "sprint_pc", "+sprint_and_cameralock", &"INTRO_HINT_SPRINT_WII" );
registerActionBinding( "sprint_pc", "+zappermenu", &"INTRO_HINT_SPRINT_WII" );
...
watch_hint_restart( actionName, timeOut, doubleline, notifyName, actionCommand )
{
level.player endon( notifyName );
level endon ( "clearing_hints" );
initialKeyBinding = getKeyBinding( actionCommand );
bindingChanged = false;
while( bindingChanged == false )
{
currentKeyBinding = getKeyBinding( actionCommand );
foreach(arrayKey,arrayValue in currentKeyBinding )
{
if(arrayValue != initialKeyBinding[arrayKey])
{
bindingChanged = true;
break;
}
...
thread hint_for_action( actionName, timeOut, doubleline, notifyName );
}
...
thread watch_hint_restart( actionName, timeOut, doubleline, notifyName, actionBind.binding );
...
return level.actionBinds[action][0];
}
