...
{
self thread turret_shell_sound();
}
}
}
turret_shell_sound()
...
for ( i = 0;i < 1;i += dif )
{
targ_org.origin = ( current_org * ( 1.0 - i ) ) +
( (level.player getorigin() + stance_num()) * i );
if ( player_safe() )
i = 2;
wait( wait_time );
}
...
mg42_target_drones( nonai, team, dotrange )
{
...
drone_target( drone, time, dotrange )
{
self endon( "death" );
drone endon( "death" );
...
if ( !BulletTracePassed( self GetTagOrigin( "tag_flash" ), drone.origin + ( 0, 0, 40 ), 0, drone ) )
{
...
if ( !BulletTracePassed( self GetTagOrigin( "tag_flash" ), ent GetCentroid(), false, ent ) )
{
ent = undefined;
continue;
}
