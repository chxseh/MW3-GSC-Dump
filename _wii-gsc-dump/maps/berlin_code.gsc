...
friends = add_to_array(friends, level.lone_star);
friends = add_to_array(friends, level.essex);
friends = add_to_array(friends, level.truck);
return friends;
}
intro_sequence()
{
vision_set_fog_changes("berlin_collapse", 0);
...
{
level.lone_star thread trigger_wait_multiple_once_from_targetname( "lone_star_near_building_throw", "lone_star_near_building_throw" );
...
guys[1] wiisetallowragdoll(true);
...
thread put_friendlies_into_sniping_pos();
thread glowy_sniper_rifle();
flag_wait( "snipe_player_in_position" );
autosave_by_name( "sniper_01" );
thread path_patrollers_around();
thread spawn_delta_support_squad();
array_thread( level.heroes , ::set_accuracy, 1.5, "sniper_complete" );
flag_wait( "bravo_team_spawned" );
thread spawn_hotel_rooftop_enemies();
thread delta_squad_movement();
...
thread spawn_ground_level_enemies();
thread spawn_sniper_tanks();
thread monitor_sniper_tanks_dead();
thread heros_fire_control_for_scene();
...
if( isAlive( level.sniper_tank_3 ) )
{
...
flag_wait( "delta_support_squad_heli_check_failure" );
...
array_thread( sniper_tank_spawners, ::add_spawn_function, ::kill_path_on_death );
...
wait( 3.7 );
if( !self.death_pending && !level.mission_failing )
{
...
{
thread maps\berlin_util::monitor_reload_event( flag_end_on );
reload_count = 0;
...
{
level.berlin_reload_amb_timer = GetTime() + 10 * 1000;
reload_count++;
switch(reload_count)
{
case 1:
...
break;
case 2:
if( !isDefined( level.allow_ambient_missiles ) || level.allow_ambient_missiles == true )
{
Acquire_AirSpaceLock();
veh_3 = spawn_vehicles_from_targetname_and_drive( "mainstreet_hind" );
array_thread( veh_3, ::mainstreet_hind_fire );
...
binocs delete();
flag_wait_or_timeout( "player_near_rappel", 5 );
...
self anim_single_solo( self, "bog_b_spotter_spot_2_casual" );
self anim_single_solo( self, "casual_crouch_idle_out" );
flag_wait_or_timeout( "player_near_rappel", 2 );
...
self AllowedStances( "stand", "crouch", "prone" );
self forceUseWeapon( self.old_weapon, "primary" );
flag_wait_or_timeout( "player_near_rappel", 1 );
...
thread maps\berlin_fx::rappel_dof();
...
flag_wait( "lone_star_sniping_position_complete" );
if( self == level.truck )
...
rappel_anim_ent thread play_rappel_anim_and_stop( actors, anim_name);
self.rappelling = undefined;
}
play_rappel_anim_and_stop( actors, anim_name )
{
self anim_single_run( actors, anim_name);
actors[0] set_goal_pos( actors[0].origin );
}
rappel_ambient_a10()
{
wait(4.5);
...
flag_wait("bridge_fighters_start_fighting");
thread bridge_fighters_wake_up( bridge_enemies );
}
while(level.bridge_fighter_dead < 6)
{
wait(.1);
}
...
while(level.bridge_fighter_dead < 8)
{
wait(.1);
}
...
maps\_spawner::killspawner(19);
maps\_spawner::killspawner(18);
...
{
rpg add_weapon_glow( "stop_rpg_glow", true );
}
...
wait( 5 );
if( !flag( "cancel_scripted_color_trigs" ) )
{
...
apache_shotdown.preferred_crash_style = 0;
...
thread monitor_bridge_battle_fail_condition( ally_tanks[0] );
...
break;
vec = get_targeting_vector(g, barrel_org);
...
else if(barrel_dot > fire_cone_cos )
{
break;
}
else
...
{
trigger_wait_targetname( "trigger_lone_star_wounded" );
...
{
tank delaythread( 2.9, ::self_delete );
}
}
}
}
...
player_rig_aftermath.weapon = "none";
...
aud_send_msg( whistle_msg, [pos, whistle_time] );
wait( whistle_time );
aud_send_msg(impact_msg, pos);
...
flag_wait_either( "player_at_ceiling_collapse", "lone_star_at_ceiling_collapse" );
...
level.scripted_tank thread last_stand_scripted_tank_fire();
...
door = GetEnt( "door_hotel_stairs_1", "targetname" );
if( IsDefined( door ) )
{
door ConnectPaths();
door Hide();
...
anim_ent thread anim_loop_solo( breach_actors[0], "reverse_breach_idle", "stop_breach_actor_idle" );
anim_ent thread anim_loop_solo( breach_actors[1], "reverse_breach_idle", "stop_breach_actor_idle" );
anim_ent thread anim_loop_solo( breach_actors[3], "reverse_breach_idle", "stop_breach_actor_idle" );
...
rel_pos = ( -14.350, -1.056, -2.094 );
rel_ang = ( 7.341, 171.453, -82.298 );
...
thread lerp_fov_overtime( 6.16, 40 );
...
delaythread( 7.9, ::reverse_breach_getup_regular_speed );
wait( length );
...
trigger_ent SetHintString( &"BERLIN_BREACH_HINT" );
flag_wait( "trigger_start_reverse_breach" );
trigger_ent Delete();
}
reverse_breach_cleanup_weapons()
...
newMissile thread fake_javelin_earthquake( self );
...
break;
case "sandman":
level.lone_star ForceTeleport( pos.origin, pos.angles );
break;
case "truck":
level.truck ForceTeleport( pos.origin, pos.angles );
break;
case "grinch":
level.essex ForceTeleport( pos.origin, pos.angles );
break;
}
}
}
