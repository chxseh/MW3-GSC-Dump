...
addforcestreamxmodel( "viewmodel_uav_control_unit" );
...
remove_uav_weapon()
{
removeforcestreamxmodel( "viewmodel_uav_control_unit" );
...
player notify( "remote_missile_exploded" );
...
thread staticEffect( undefined, true );
rig = player_uav_rig();
...
wait 0.05;
player.remote_missile_grain_overlay = create_client_overlay( "ac130_overlay_grain", 1, player );
player.remote_missile_grain_overlay.foreground = false;
player.remote_missile_grain_overlay.sort = -5;
...
{
missile thread explode_early_under_height();
}
...
if( level.script == "ny_manhattan_b" )
{
player thread cover_up_till_ground_streamed();
}
...
cover_up_till_ground_streamed()
{
level.missile_white_fade_out = maps\_hud_util::create_client_overlay( "white", 1, self );
wait ( 0.5 );
if( IsDefined( level.missile_white_fade_out ) )
{
level.missile_white_fade_out fade_over_time( 0, 1.8 );
}
if( IsDefined( level.missile_white_fade_out ) )
{
level.missile_white_fade_out Destroy();
}
}
explode_early_under_height()
{
while( IsDefined( self ) )
{
if( self.origin[2] < level.remote_missile_detonate_height )
{
self notify( "death" );
break;
}
...
{
level.missile_white_fade_out Destroy();
level.missile_white_fade_out = undefined;
}
player.is_flying_missile = undefined;
if (isdefined(level.firemissile_uav_hud_cb))
{
...
DrawTargetsEnd();
if( IsDefined( player.remote_missile_grain_overlay ) )
{
player.remote_missile_grain_overlay Destroy();
}
staticEffect( undefined, true );
if ( IsDefined( level.uav_is_destroyed ) )
...
if (!level.console)
{
player NotifyOnPlayerCommand( "abort_remote_missile", "+gostand" );
player NotifyOnPlayerCommand( "abort_remote_missile", "togglecrouch" );
player NotifyOnPlayerCommand( "abort_remote_missile", "toggleprone" );
}
player NotifyOnPlayerCommand( "launch_remote_missile", "+attack" );
player NotifyOnPlayerCommand( "launch_remote_missile", "+attack_akimbo_accessible" );
...
staticEffect( duration, geomStreamVersion )
{
...
if( IsDefined( geomStreamVersion ) )
{
...
if( IsDefined( geomStreamVersion ) )
{
geoBlack = create_client_overlay( "black", 1 );
geoBlack.foreground = 0;
static.alpha = 1;
static.sort = 2;
do_wiigeomstreamnotify(1,1,0,1,1);
geoBlack Destroy();
}
else
{
