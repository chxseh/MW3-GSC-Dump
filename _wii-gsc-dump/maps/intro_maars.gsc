...
thread maps\intro_fx::maars_hide_building();
...
bar = self createClientProgressBar( self, 80, "white", "black", 300, 10 );
...
{
return( ( ( level.line_offset ) * level.feedline_height ) + 30 );
}
...
yaw = AngleClamp180( player_angles[ 1 ] );
...
level.player DigitalDistortSetParams( newdistort_intensity, newdistort_speed );
wait update_time * 2;
...
level.player DigitalDistortSetParams(distort_intensity,(1+2.0-distort_intensity*2.0));
...
return;
self endon( "death" );
if ( IsDefined( level.player_drone ) )
{
if ( !IsAlive( level.player_drone ) )
return;
level.player_drone endon( "death" );
}
MYFADEINTIME = 1.0;
MYFLASHTIME = 0.75;
MYALPHAHIGH = 0.95;
