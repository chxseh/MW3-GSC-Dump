...
self.player uav_set_target_hud_data( "brg", AngleClamp( int( ( self.angles[ 1 ] - 90 ) ) * -1 ) );
...
num_table_entries = 7;
scaled_index = abs( latitude ) * num_table_entries / 90.0;
...
data = [];
data[ "brg" ] = [ &"UAV_BRG", "" ];
data[ "rng_m" ] = [ &"UAV_RNG", &"UAV_M" ];
data[ "rng_nm" ] = [ &"UAV_RNG", &"UAV_NM" ];
data[ "elv" ] = [ &"UAV_ELV", &"UAV_F" ];
