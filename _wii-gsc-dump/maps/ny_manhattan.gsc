...
{
PreCacheModel( "vehicle_mi24p_hind_manhattan" );
...
maps\sp_ny_manhattan_precache::main();
...
PreCacheString( &"NY_MANHATTAN_PREDATOR_WIIMOTE" );
...
add_start( "Hind Ride",::start_hind_ride_flight, "Hind Ride",::hind_ride_flight );
add_start( "Hind Finale",::start_hind_finale, "Hind Finale",::hind_finale );
}
...
{
vehSpawners = Vehicle_GetSpawnerArray();
foreach( spawner in vehSpawners )
{
if( spawner.model == "vehicle_mi24p_hind_woodland" )
spawner thread add_spawn_function( ::wii_use_manhattan_hind );
}
}
}
wii_use_manhattan_hind()
{
self SetModel( "vehicle_mi24p_hind_manhattan" );
}
...
{
wait( 3 );
aud_send_msg( "play_random_temp_vo", 5 );
}
...
thread tiff_main();
thread wait_for_player_entering_hind();
...
wait 0.2;
level.player_hind notify("newpath");
level.hind_battle_hind03c notify("newpath");
