...
chopper delete();
autosave_by_name( "chopper_crash" );
wait( 2 );
...
obj_rope = get_target_ent( "player_rappel_bar" );
obj_rope Show();
bar = get_target_ent( "player_rappel_bar_normal" );
bar Hide();
...
level.player PlayerLinkToDelta( rig, "tag_player", 1, 0, 0, 40, 5 );
ai = getaiarray( "axis" );
array_call( ai, ::delete );
...
autosave_by_name( "cavern_defend" );
...
delayThread( animtime*0.8, ::flag_set, "ending_next_stop" );
...
level.rescue_heli notify( "stop_kicking_up_dust" );
...
level.president_holder wiisetallowragdoll(true);
...
holder_fired( guy )
{
level.president dodamage(5, guy.origin);
}
fail_on_death()
{
