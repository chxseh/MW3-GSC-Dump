...
silo_interior_visibility( false );
...
{
thread debug_water_swap( ents[ 0 ].script_noteworthy, hide_them );
if (!isdefined(ents))
return;
foreach (ent in ents)
{
...
thread start_bobbing( bobbing_mines );
}
InitBobbingVolumes()
{
...
level.zodiac_rumble delaythread ( 4.2, ::rumble_ramp_to, 0, 1 );
}
play_ssn19fx( name )
{
wait(0.95);
...
missile thread play_ssn19fx( sn_name );
...
snode.origin = node.origin;
snode.angles = node.angles;
thread ladder_cleanup();
...
anim_ent.origin = (-38581, -23711, -416.5);
door = spawn_anim_model( "door", anim_ent.origin );
level.sub_doors[ level.sub_doors.size ] = door;
anime = "slam_door" ;
...
debug_water_swap( strName, boolHideThem )
{
}
ladder_cleanup()
{
...
( vehicle.model == "ch_46e_ny_harbor" )
)
{
vehicle Delete();
continue;
