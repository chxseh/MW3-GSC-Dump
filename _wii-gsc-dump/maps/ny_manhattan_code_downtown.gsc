...
nyse_split_transition()
{
if( is_split_level_part( "a" ) )
{
trig = Spawn( "trigger_radius", (-336, 1984, -4), 0, 99.18, 128 );
trig waittill( "trigger" );
maps\_loadout::SavePlayerWeaponStatePersistent( "ny_manhattan_a", true );
fadeOutTime = 1;
...
scriptednode thread anim_single_solo( door, "closed" );
...
flag_wait ( "nyse_rooftop_hind" );
level thread do_forcestreamerdefrag();
...
level.thermite hide();
level.thermite maps\_c4::c4_location( undefined, ( 0, 0, 0 ), ( 0, 0, 0 ), level.thermite.origin, "weapon_thermite_device", "weapon_thermite_device_obj" );
...
thread jammer_destruct_react();
thread jammer_destruct_dialog();
wait 2.5;
wait 0.5;
org thread anim_last_frame_solo( jammer_destroyed, "collapse" );
}
...
thread blackhawk_approach_monitor_a_button();
org anim_single_solo ( level.player_hind, "approach" );
org thread anim_loop_solo ( level.player_hind, "approach_idle", "stop_idle" );
level.player_hind SetAnim( level.scr_anim[level.player_hind.animname]["approach"], 0, 0.0 );
level.player_hind SetAnim( level.scr_anim[level.player_hind.animname]["approach_idle"][0], 1, 0.0 );
...
array_thread( GetEntArray( "enemy_predator_01", "targetname" ), ::self_delete );
array_thread( GetEntArray( "enemy_predator_02", "targetname" ), ::self_delete );
...
level.player thread maps\_remotemissile::disable_uav( false, true );
...
{
self endon("death");
start_forward = (0.0111, -0.342, -0.94);
while( !flag ( "predator_defend_complete" ) )
{
...
pos = self.active_uav_missile.origin;
self DoDamage(1,pos);
break;
}
wait 0.05;
...
if( !flag( "uav_in_use" ) )
{
thread display_hint ("hint_predator");
}
