...
self thread local_fix_for_aiming_dir_bug();
...
if( !isdefined( ocean_vista ) || !isdefined( ocean_initial ) )
{
if( !isdefined( ocean_vista ) )
{
println("ocean_vista not defined");
}
if( !isdefined( ocean_initial ) )
{
println("ocean_initial not defined");
}
return;
}
ocean_vista hide();
...
player_rig setmodel( "viewhands_player_pmc_dubai" );
...
flag_wait( "replacement_elevator_in_position" );
org_replacement_contents = replacement_mainframe SetContents( 0 );
thread elevator_yuri_handle_collision();
elevator_player_jump_node.origin = elevator_player_jump_node.origin + (0,0,100);
elevator_player_jump_node linkto( starting_mainframe );
level waittill( "player_elevator_jump_successul" );
time_in_air = 1000*(26.0/30.0);
...
{
starting_node.origin = starting_node.origin + (0,0,100);
...
replacement_mainframe SetContents( org_replacement_contents );
}
elevator_yuri_handle_collision()
{
wait 1;
yuri_contents = level.yuri SetContents( 0 );
flag_wait( "player_finished_jump_to_replacement_elevator" );
level.yuri SetContents( yuri_contents );
}
yellowdotoff()
{
...
blend_time = 0.75;
level.player PlayerLinkToBlend( player_rig, "tag_player", blend_time );
level.player delaythread ( 5.5, ::yellowdotoff );
...
elevator_player_jump_node anim_single_solo( actors[0], anime );
...
level.hudelem.label = &"DUBAI_MAKAROV_ESCAPING";
...
restaurant_destruction_player_start_node thread anim_single( actors, anime );
anim_time = GetAnimLength( level.scr_anim[ "player_rig" ][ "restaurant_destruction" ] );
wait( anim_time - 8.0 );
player_legs hide();
restaurant_destruction_player_start_node waittill( anime );
...
if( !isdefined(restaurant_glass) )
{
println("restaurant_glass not defined");
return;
}
...
self waittill("death");
maps\_shg_fx::kill_exploder( exploderid );
}
pulse()
{
...
if ( (!IsDefined( level.credits_active) || !level.credits_active ) && !using_wii() )
{
