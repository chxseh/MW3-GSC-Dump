...
new_destination = friend_smoke_beach.origin + ( vector * 500 );
wait 0.15;
...
delaythread( 2.1, ::event1_encounter2 );
...
return;
level endon ( current_trigger.script_parameters );
}
...
hot_help_heli = get_vehicle( "crane_apache", "targetname" );
if( isdefined( hot_help_heli ) )
{
println("crane_apache defined!");
hot_help_heli.enableRocketDeath = true;
hot_help_heli godoff();
hot_help_helis[ hot_help_helis.size ] = hot_help_heli;
}
current_hot_help = 0;
while( current_hot_help < hot_help_helis.size )
{
foreach( spot in javelin_to_hot_help_helis )
{
println("current_hot_help " + current_hot_help);
...
flag_wait( "two_tanks_in_alley" );
two_tanks_in_alley = GetEnt( "two_tanks_in_alley", "targetname" );
...
{
missile_chain( targetname, undefined, undefined, "mi28_zippy" );
}
do_tank_mount()
{
...
level.player_tank Vehicle_Teleport( node.origin, CombineAngles( node.angles, ( 0, 180, 0 ) ) );
...
level.player EnableInvulnerability();
level.player.stayinvulnerable = true;
...
player_rig Delete();
level.player.stayinvulnerable = false;
...
array_thread( axis_ai, ::safe_set_ignoreall, true );
array_thread( axis_ai, ::safe_set_ignoreme, true );
array_thread( axis_ai, ::delaythread, 6.5, ::safe_set_ignoreall, false );
array_thread( axis_ai, ::delaythread, 6.5, ::safe_set_ignoreme, false );
}
safe_set_ignoreall( val )
{
if( isdefined( self ) && IsAlive( self ) )
{
self set_ignoreall( val );
}
}
safe_set_ignoreme( val )
{
if( isdefined( self ) && IsAlive( self ) )
{
self set_ignoreme( val );
}
}
ride_in_dialogue()
{
...
thread radio_dialog_add_and_go( "hamburg_snd_nevermadeit" );
