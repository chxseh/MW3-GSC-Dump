...
playerForward = VectorNormalize( AnglesToForward( self GetPlayerAngles() ) );
...
nearEnd = min( distFrom - 30, nearEnd );
farStart = max( distFrom + 30, farStart );
}
if ( nearEnd > farStart )
{
nearEnd = 256;
...
{
nearEnd = max( 50, min( 512, nearEnd ) );
farStart = max( 1000, min( 2500, farStart ) );
}
traceDist = Distance( playerEye, trace[ "position" ] );
if ( nearEnd > traceDist )
nearEnd = max( 1, traceDist - 30 );
farStart = max( traceDist, farStart );
