...
zodiac_preLoad( playerHandModel, playerZodiacModel )
{
...
if (!isdefined(playerZodiacModel))
level.zodiac_playerZodiacModel = "vehicle_zodiac_viewmodel";
else
level.zodiac_playerZodiacModel = playerZodiacModel;
...
if (!isdefined(noshoot) || !isdefined(noprice))
player thread drive_target_enemy( vehicle, noprice, noshoot );
player thread drive_crash_detection( vehicle );
player thread drive_camera( vehicle, noshoot );
...
player drive_switch_to_1st_person( vehicle, noshoot );
...
drive_target_enemy( vehicle, noprice, noplayer )
{
assert(!isdefined(noprice) || !isdefined(noplayer));
vehicle endon( "vehicle_dismount" );
vehicle endon( "death" );
vehicle endon( "stop_targetting" );
check_dist_for_hargroves_boat = [];
check_dist_for_enemy_boat = [];
check_dist_for_stationary_guys= [];
ai_get_func= [];
baseYawSettings = [];
if (!isdefined(noplayer))
{
...
self drive_switch_to_1st_person( vehicle, noshoot );
}
}
...
drive_switch_to_1st_person( vehicle, noshoot )
{
...
self thread drive_firstperson_anims( vehicle, noshoot );
}
stop_1st_person( vehicle )
{
if ( !isDefined( vehicle.firstPerson ) )
return;
...
drive_firstperson_anims( vehicle, noshoot )
{
vehicle endon( "vehicle_dismount" );
vehicle endon( "death" );
vehicle endon( "kill_anims" );
vehicle endon( "cleanup" );
self childthread drive_shooting_anims( vehicle, noshoot );
}
...
continue;
if ( isdefined(level.enemy_boat) && ( boat == level.enemy_boat ) )
continue;
if ( isdefined(level.ally_boat) && ( boat == level.ally_boat ) )
continue;
if ( IsSpawner( boat ) )
continue;
...
drive_shooting_anims( vehicle, noshoot )
{
